(function ($) {
  ...
  $.fn.scrollsnap = function (options) {
    var settings = $.extend({
      ...
      'latency': 250,
      'easing': 'swing',
      'onSnapEvent': 'scrollsnap',
      // triggered on the snapped DOM element
      'onSnap': function ($snappedElement, silent) {
      },
      // callback when an element was snapped
      'onSnapWait': 50  // wait for redundant snaps before firing event / calling callback
    }, options);
    var leftOrTop = settings.direction === 'x' ? 'Left' : 'Top', offsetLT = 'offset' + leftOrTop, scrollLT = 'scroll' + leftOrTop;
    return this.each(function () {
      var
        scrollingEl = this, $scrollingEl = $(this),
        lastOffset,
        handler;
      if (scrollingEl.nodeType === 1) {
        ...
        handler = function (e) {
          ...
          $scrollingEl.find(settings.snaps).each(function () {
            var snappingEl = this, dy = Math.abs(snappingEl[offsetLT] + settings.offset - scrollingEl[scrollLT]);
            if (dy < matchingDy) {
          ...
          if (matchingEl) {
            var endScroll = matchingEl[offsetLT] + settings.offset, animateProp = {}, $matchingEl = $(matchingEl);
        ...
        handler = function (e) {
          ...
          $(scrollingEl).find(settings.snaps).each(function () {
            ...
            var snappingEl = this, dy = Math.abs($(snappingEl).offset()[leftOrTop.toLowerCase()] + settings.offset - win[tempOffset]);
            if (dy < matchingDy) {
              matchingEl = snappingEl;
              matchingDy = dy;
            }
          ...
          if (matchingEl) {
            var $matchingEl = $(matchingEl), endScroll = $matchingEl.offset()[leftOrTop.toLowerCase()] + settings.offset, animateProp = {};
            animateProp[scrollLT] = endScroll;
            if (Math.abs($scrollingEl[scrollLT]() - endScroll) > 2) {
